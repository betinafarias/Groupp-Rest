...
--
-- Host: localhost
-- Generation Time: Nov 21, 2016 at 02:24 AM
...
INSERT INTO `musician` (`id`, `name`, `password`, `email`, `age`, `id_city`) VALUES
(1, 'Betina', '1234', 'betina@gmail.com', 21, 4174),
(4, 'Bob Marley', '1234', 'bob@gmail.com', 43, 4174);

-- --------------------------------------------------------

--
-- Stand-in structure for view `musician_full`
--
CREATE TABLE IF NOT EXISTS `musician_full` (
`id` int(11)
,`name` varchar(50)
,`password` varchar(30)
,`email` varchar(120)
...
--
-- Structure for view `musician_full`
--
DROP TABLE IF EXISTS `musician_full`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `musician_full` AS select `musician`.`id` AS `id`,`musician`.`name` AS `name`,`musician`.`password` AS `password`,`musician`.`email` AS `email`,`musician`.`age` AS `age`,`musician`.`id_city` AS `id_city`,`instrument`.`name` AS `instrument_name`,`cidade`.`nome` AS `city_name`,`genres`.`name` AS `genre_name` from (((((`musician` join `musician_instrument` on((`musician_instrument`.`id_musician` = `musician`.`id`))) join `instrument` on((`musician_instrument`.`id_instrument` = `instrument`.`id`))) join `cidade` on((`cidade`.`id` = `musician`.`id_city`))) join `musician_genre` on((`musician_genre`.`id_musician` = `musician`.`id`))) join `genres` on((`genres`.`id` = `musician_genre`.`id`)));
